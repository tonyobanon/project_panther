...
import com.re.paas.api.roles.AbstractSlaveRole;
...
public class IngestAdapterConfigFunction
	...
	public Class<? extends AbstractRole> role() {
		return AbstractSlaveRole.class;
	}
	...
	public IngestAdapterConfigResponse delegate(IngestAdapterConfigRequest t) {
		...
		for (AdapterConfig config : t.getAdapterConfig()) {
			...
			try {
				status = delegate.load(LoadPhase.START);
			} catch (Exception e) {
				status = e;
			}
			
			response.addStatus(config.getType(), status);
		}
