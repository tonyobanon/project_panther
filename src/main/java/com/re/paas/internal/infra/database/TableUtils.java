...
class TableUtils {
	...
	public static TableDefinition toTableDefinition(Map<String, String> schema, Class<? extends BaseTable> entity) {
		...
		if (!Collections.hasUniqueElements(textSearchRangeKeys)) {
			...
			// When a CRUD operation happens on the tables, AttributeModelImpl iterates
			// through all attributes, looking for any one that match the range key of a
			// text search GSI - after which the partition size of the index.
		...
		// Add every other class field on the table, as an attribute
