...
import com.re.paas.api.listable.search.DocValuesType;
...
public class FieldType implements IndexableFieldType {
	...
	public FieldType(IndexableFieldType ref) {
		...
		this.indexOptions = IndexOptions.valueOf(ref.indexOptions().name());
		this.docValuesType = DocValuesType.valueOf(ref.docValuesType().name());
	 ...
	 * <code>ref</code>
	 */
	public FieldType(org.apache.lucene.index.IndexableFieldType ref) {
		this.stored = ref.stored();
		this.tokenized = ref.tokenized();
		this.storeTermVectors = ref.storeTermVectors();
		this.storeTermVectorOffsets = ref.storeTermVectorOffsets();
	...
	public org.apache.lucene.document.FieldType asLuceneFieldType() {
		
		org.apache.lucene.document.FieldType ref = new org.apache.lucene.document.FieldType();
		
		ref.setStored(this.stored());
		ref.setTokenized(this.tokenized());
		ref.setStoreTermVectors(this.storeTermVectors());
		ref.setStoreTermVectorOffsets(this.storeTermVectorOffsets());
		ref.setStoreTermVectorPositions(this.storeTermVectorPositions());
		ref.setStoreTermVectorPayloads(this.storeTermVectorPayloads());
		ref.setOmitNorms(this.omitNorms());
		ref.setIndexOptions(org.apache.lucene.index.IndexOptions.valueOf(ref.indexOptions().name()));
		ref.setDocValuesType(org.apache.lucene.index.DocValuesType.valueOf(ref.docValuesType().name()));
		
		ref.setDimensions(this.pointDataDimensionCount(), this.pointIndexDimensionCount(), this.pointNumBytes());
	
		if (ref.getAttributes() != null) {
			for(Entry<String, String> e : ref.getAttributes().entrySet()) {
				ref.putAttribute(e.getKey(), e.getValue());	
			}
		}
		return ref;
	}
	...
	/**
	 * Throws an exception if this FieldType is frozen. Subclasses should call this
	 * within setters for additional state.
	 */
	...
	/**
	 * Prevents future changes. Note, it is recommended that this is called once the
	 * FieldTypes's properties have been set, to prevent unintentional state
	 * changes.
	 */
	public void freeze() {
		this.frozen = true;
	 ...
	 * @throws IllegalStateException if this FieldType is frozen against future
	 *                               modifications.
	 * @see #stored()
	 */
	...
	/**
	 * Set to <code>true</code> to tokenize this field's contents via the configured
	 * {@link Analyzer}.
	 * 
	...
	/**
	 * Set to <code>true</code> to also store token positions into the term vector
	 * for this field.
	 * 
	...
	/**
	 * Set to <code>true</code> to also store token payloads into the term vector
	 * for this field.
	 * 
	...
	public void setDimensions(int dataDimensionCount, int indexDimensionCount, int dimensionNumBytes) {
		...
		if (dataDimensionCount > PointValues.MAX_DIMENSIONS) {
			throw new IllegalArgumentException(
					"dataDimensionCount must be <= " + PointValues.MAX_DIMENSIONS + "; got " + dataDimensionCount);
		}
		...
		if (indexDimensionCount > dataDimensionCount) {
			throw new IllegalArgumentException("indexDimensionCount must be <= dataDimensionCount: "
					+ dataDimensionCount + "; got " + indexDimensionCount);
		}
		...
		if (dimensionNumBytes > PointValues.MAX_NUM_BYTES) {
			throw new IllegalArgumentException(
					"dimensionNumBytes must be <= " + PointValues.MAX_NUM_BYTES + "; got " + dimensionNumBytes);
		}
		if (dataDimensionCount == 0) {
			if (indexDimensionCount != 0) {
				throw new IllegalArgumentException(
						"when dataDimensionCount is 0, indexDimensionCount must be 0; got " + indexDimensionCount);
			}
			if (dimensionNumBytes != 0) {
				throw new IllegalArgumentException(
						"when dataDimensionCount is 0, dimensionNumBytes must be 0; got " + dimensionNumBytes);
			}
			...
			if (dataDimensionCount != 0) {
				throw new IllegalArgumentException(
						"when dimensionNumBytes is 0, dataDimensionCount must be 0; got " + dataDimensionCount);
			}
		}
	 ...
	 * value. This method is not thread-safe, user must not add attributes while
	 * other threads are indexing documents with this field type.
	 *
	 */
	...
	public boolean equals(Object obj) {
		...
		if (dataDimensionCount != other.dataDimensionCount)
			return false;
		if (indexDimensionCount != other.indexDimensionCount)
			return false;
		if (dimensionNumBytes != other.dimensionNumBytes)
			return false;
		if (docValuesType != other.docValuesType)
			return false;
		if (indexOptions != other.indexOptions)
			return false;
