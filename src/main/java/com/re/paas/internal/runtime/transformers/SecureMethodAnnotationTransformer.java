		...
		for (Entry<String, List<String>> e : classes.entrySet()) {
			...
			for (Method m : methodsArray) {
				...
				if (Modifier.isStatic(m.getModifiers()) || Modifier.isFinal(m.getModifiers())
						|| (!ClassTools.hasAnnotation(m, SecureMethod.class))) {
					continue;
				}
			...
			try {
						...
						for (Method m : methods) {
							...
							try {
								...
								if (!Modifier.isAbstract(m.getModifiers())) {
									...
									ClassTools.copyAnnotation(target, source, m, annotationTypes);
									transformed = true;
								}
				...
				for (Method m : methods) {
					...
					if (Modifier.isAbstract(m.getModifiers())) {
						...
						ClassTools.removeAnnotation(source, m, SecureMethod.class);
