...
public class FileSystemProviders {
	...
	private static void initInternalFs() {
		...
		try {
		if(os.indexOf("win") >= 0) {
			internalFsProviderClass = (Class<FileSystemProvider>) cl.loadClass("sun.nio.fs.WindowsFileSystemProvider");
		} else if(os.indexOf("mac") >= 0) {
			internalFsProviderClass = (Class<FileSystemProvider>) cl.loadClass("sun.nio.fs.MacOSXFileSystem");
		} else if(os.indexOf("sunos") >= 0) {
			internalFsProviderClass = (Class<FileSystemProvider>) cl.loadClass("sun.nio.fs.SolarisFileSystemProvider");
			...
			internalFsProviderClass = (Class<FileSystemProvider>) cl.loadClass("sun.nio.fs.LinuxFileSystemProvider");
