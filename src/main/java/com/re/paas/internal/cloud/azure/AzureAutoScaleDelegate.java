...
public class AzureAutoScaleDelegate extends AutoScaleDelegate {
	...
	public InstanceProfile getInstanceProfile() {
		...
		try {
			...
			String vmSizeType = MasterNodeConfig.getInstance().get(MasterNodeConfig.AZURE_VIRTUAL_MACHINE_SIZE_TYPE);
		...
		try {
			...
			case LINUX:
				...
				withCreate = withOS.withStoredLinuxImage(iProfile.getImage()).withRootUsername(username)
						.withRootPassword(password).withSsh(keypair.getPublicKey()).withComputerName(resourceName);
			...
			VirtualMachine vm = withCreate.withSize(iProfile.getInstanceTypeSpec())
					.withTag(Tags.RESOURCE_NAME_TAG, resourceName).withTag(Tags.MASTER_TAG, master.toString())
			...
			addInstanceCredential(credential);
