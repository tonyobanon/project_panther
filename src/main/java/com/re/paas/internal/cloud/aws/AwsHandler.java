...
public class AwsHandler extends AbstractProviderHandler {
	...
	public InstanceProfile getInstanceProfile() {
		...
		try {
			...
			String instanceType = env.getInstanceTags().get(AwsTags.AWS_INSTANCE_TYPE);
	...
	public InstanceCredential startVM(Boolean master, Map<String, String> tags) {
		...
		try {
			...
			amazonVM.withUserData(AWSHelper.toUserData(tags));
			...
			List<Tag> ec2Tags = new ArrayList<>(tags.size());
			
			tags.forEach((k, v) -> {
				ec2Tags.add(new Tag(k, v));
			});

			// Include a tag for this the resource name, in this instance
			ec2.createTags(new CreateTagsRequest().withResources(instanceId).withTags(ec2Tags));
