	...
	public PdfDocumentSpec generateSpec(Path path) throws IOException, CertificateException, NoSuchAlgorithmException,
		...
		try (PDDocument document = PDDocument.load(content)) {
			...
			for (PDSignature sig : document.getSignatureDictionaries()) {
				...
				case PdfSignatureConstants.SUBFILTER_ADBE_PKCS7_DETACHED:
				...
				case PdfSignatureConstants.SUBFILTER_ADBE_PKCS7_SHA1: {

					verifyPKCS7(signedContent, sigBlock, sigInfo.setDetached(false));
					break;
				}

				case PdfSignatureConstants.SUBFILTER_ETSI_CADES_DETACHED: {

					verifyPKCS7(signedContent, sigBlock, sigInfo.setDetached(true));
					break;
				}

				case PdfSignatureConstants.SUBFILTER_ADBE_X509_RSA_SHA1: {
					Throwable t = new UnsupportedOperationException(
							"Subfilter: " + PdfSignatureConstants.SUBFILTER_ADBE_X509_RSA_SHA1 + " not yet supported");
					Exceptions.throwRuntime(t);
					break;
				}

				case PdfSignatureConstants.SUBFILTER_ETSI_RFC3161: {
					Throwable t = new UnsupportedOperationException(
							"Subfilter: " + PdfSignatureConstants.SUBFILTER_ETSI_RFC3161 + " not yet supported");
					Exceptions.throwRuntime(t);
					break;
				}
