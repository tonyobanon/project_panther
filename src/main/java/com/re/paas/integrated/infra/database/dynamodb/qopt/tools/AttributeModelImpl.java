...
import com.re.paas.integrated.infra.database.dynamodb.AttributeModel;
...
import com.re.paas.integrated.infra.database.dynamodb.qopt.classes.PartitionSizeExceeded;
...
public class AttributeModelImpl implements AttributeModel {
	...
	public void putValue(String tableName, Item item) {
		...
		for (Entry<String, Object> e : item.attributes()) {
			...
			if (rangeKeyToIndexName.containsKey(tableName + "/" + indexRangeKey)) {
				...
				Item indexEntry = Database.get().lean().getTable(IndexEntrySpec.TABLE_NAME).getItem(spec);
	...
	private void addIndexEntry(IndexDescriptor index, TablePrimaryKey tableKey, int itemSize) {
		...
		if (tableKey.getRangeKey() != null) {
			...
			Database.get().lean().getTable(index.getTableName())
		...
		Database.get().lean().getTable(IndexEntrySpec.TABLE_NAME)
	...
	private void updateIndexEntrySize(IndexDescriptor index, TablePrimaryKey tableKey, int itemSize) {
		...
		Table indexEntryTable = Database.get().lean().getTable(IndexEntrySpec.TABLE_NAME);
		...
		if (sizeOffset < 0) {
			...
			try {
				...
				if (tableKey.getRangeKey() != null) {
					...
					Database.get().lean().getTable(index.getTableName()).updateItem(
	...
	public void deleteValue(String tableName, Object tableHashValue, String indexName) {
		...
		indexEntryTable.query(qSpec).forEach(result -> {
