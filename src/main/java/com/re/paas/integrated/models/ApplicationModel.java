...
import com.re.paas.integrated.models.helpers.FormFactory;
import com.re.paas.integrated.models.helpers.FormFieldRepository;
import com.re.paas.integrated.models.helpers.FormFieldRepository.FormField;
import com.re.paas.integrated.tables.defs.users.ApplicationFormValueTable;
import com.re.paas.integrated.tables.defs.users.ApplicationTable;
import com.re.paas.integrated.tables.defs.users.DeclinedApplicationTable;
...
public class ApplicationModel extends BaseModel {
	...
	private static final String APPLICATION_ID_SEARCH_FIELD = "APPLICATION_ID";
	private static final String PERSON_NAME_SEARCH_FIELD = "PERSON_NAME";
	...
	private static ListableIndex<?> getListableIndex(String role) {
		Realm realm = RoleModel.getRealm(role);
		return realm.applicationSpec().getListableIndex();
	}
	...
	public static Long newApplication(String role) {
		...
		SearchModel.addCachedListKey(getListableIndex(role),
	...
	public static void updateApplication(Long applicationId, Map<String, String> values) {
		...
		if (status.equals(ApplicationStatus.CREATED)) {
			...
			SearchModel.removeCachedListKey(getListableIndex(role),
					FluentArrayList.asList(new ListingFilter("status", ApplicationStatus.CREATED.getValue())),
					applicationId);
		...
		if (applicationSpec.isAutoAccept()) {
			...
			Sentence activity = Sentence.newInstance().setSubject(getPersonName(applicationId, realm))
	...
	public static Long submitApplication(Long applicationId) {
		...
		// Add to search index
		String searchIndex = getListableIndex(role).asString();

		AbstractDocument document = new DocumentImpl()
				.add(APPLICATION_ID_SEARCH_FIELD, applicationId.toString(), TextFieldType.TYPE_STORED)
				.add(PERSON_NAME_SEARCH_FIELD, getPersonName(applicationId, realm, values), TextFieldType.TYPE_STORED);

		Search2Model.addDocument(searchIndex, document);
	...
	private static void generatePDFQuestionnaire(Realm realm) {
		...
		FormModel.listSections(FormSectionType.APPLICATION_FORM, realm).forEach((section) -> {
			...
			section.withFields(FormModel.getFields(FormSectionType.APPLICATION_FORM, section.getId()).values());
		...
		FormFactory.toPDF(LocaleModel.defaultLocale(), new SizeSpec(4), new SizeSpec(5), new SizeSpec(3), form,
				getFormPath(realm));
	...
	public static Map<String, String> getFieldValues(Long applicationId, Collection<String> fieldIds) {
		...
		Collection<Item> items = Database.get().getTable(ApplicationFormValueTable.class)
				...
				.all(QuerySpec.get(ApplicationFormValueTableSpec.APPLICATION_ID, applicationId.toString(),
	...
	@BlockerTodo
	public static Map<Long, String> getApplicationRoles(Collection<Long> ids) {
		return null;
	}
	...
	public static Long acceptApplication(Long principal, Long applicationId) {
		...
		// Remove from search index: IndexRepository.{ADMIN || USER }_APPLICATION
		Search2Model.deleteDocument(getListableIndex(role).asString(),
				Arrays.asList(new TermImpl(APPLICATION_ID_SEARCH_FIELD, applicationId.toString())));

		// Add to search index:IndexRepository.USER
		AbstractDocument document = new DocumentImpl()
				.add(BaseUserModel.USER_ID_SEARCH_FIELD, userId.toString(), TextFieldType.TYPE_STORED)
				.add(BaseUserModel.PERSON_NAME_SEARCH_FIELD, BaseUserModel.getPersonName(userId, true).toString(), TextFieldType.TYPE_STORED);
		Search2Model.addDocument(IndexRepository.USER, document);
...
//	}
	...
	/**
	 * This returns a suitable application title that should be used for search
	 * indexing purpose
	 * 
	 * @param applicationId
	 * @param realm
	 * @return
	 */
	public static String getPersonName(Long applicationId, Realm realm) {
		return getPersonName(applicationId, realm, null);
	}
	 ...
	 * @return
	 */
	private static String getPersonName(Long applicationId, Realm realm, Map<String, String> fieldValues) {
		...
		StringBuilder name = new StringBuilder(fieldValues.get(firstNameField));

		if (fieldValues.get(middleNameField) != null) {
			name.append(" " + fieldValues.get(middleNameField));
		}

		name.append(" " + fieldValues.get(lastNameField));
